...
class Writer(object):
    ...
    def write_wangyi_following(self, WY_followers, WY_id):
        '''
        将WY的关注者写入数据库中
        ...
        for user in followers:
            ...
            temp_sql = "insert into 网易云关注列表新增 (`序号`,`关注用户`,`关注ID`,`网易云id`) VALUES ({},{},{},{});".format(
                index,"'{}'".format(follow_name),int(follow_id),WY_id)
    ...
    def write_wangyi_fans(self, WY_fans, WY_id):
        '''
        ...
        for user in fans:
            ...
            temp_sql = "insert into 网易云粉丝列表新增 (`index`,`name`,`粉丝ID`,`网易云ID`) VALUES ({},{},{},{});".format(
                index,"'{}'".format(fan_name),int(fan_id),WY_id)
            self.cursor.execute(temp_sql)

        self.db.commit()
        logging.info("网易云粉丝写入数据库完毕！！！")

    def write_wangyi_attributes(self, WY_user_attributes, WY_id):
        '''
        ...
        sqlStr = "insert into 网易云用户属性信息新增 " \
                 ...
                 "`网易云粉丝数`,`网易云个人简介`,`所在地区`,`用户性别`,`用户年龄`,`微博链接`,`网易云id`) values (" \
                 "{},{},{},{},{},{},{},{},{},{},{},{});".format(
            index,"'{}'".format(user_name),int(user_rank),int(user_event),int(user_follow),
            int(user_fans),"'{}'".format(user_intro),"'{}'".format(user_loc),"'{}'".format(user_sex),"'{}'".format(user_age),int(weibo_id), WY_id
        )
    ...
    def write_wangyi_posts(self, WY_posts, WY_id):
        '''
        ...
        sqlStr = "insert into 网易云用户发帖新增 (`序号`,`网易云用户名`,`网易云发帖`,`网易云id`) values (" \
                 "{},{},{},{});".format(index, "'{}'".format(name), "'{}'".format(WY_posts), WY_id)
        self.cursor.execute(sqlStr)
        self.db.commit()
        logging.info("网易云发帖写入数据完毕！！！")

    def write_weibo_following(self,WB_follow, WB_id):
        '''
        ...
        for user in follows:
            ...
            temp_sql = "insert into 微博关注列表新增 (`序号`,`name`,`用户ID`,`关注ID`) values (" \
                       "{},{},{},{});".format(index, "'{}'".format(follow_name),WB_id, follow_id)
            self.cursor.execute(temp_sql)
        self.db.commit()
        logging.info("微博关注用户写入完毕！！！")

    def write_weibo_fans(self,WB_fans, WB_id):
        '''
        ...
        for user in fans:
            ...
            temp_sql = "insert into 微博粉丝列表新增 (`序号`,`name`,`用户ID`,`粉丝ID`) values (" \
                       "{},{},{},{});".format(index, "'{}'".format(fan_name),WB_id, fan_id)
            self.cursor.execute(temp_sql)
        self.db.commit()
        logging.info("微博关注用户写入完毕！！！")
    ...
    def write_weibo_attributes(self, WB_attributes, WB_id):
        '''
        ...
        sqlStr = "insert into 微博用户信息新增 (`序号`,`用户ID`,`用户名`,`粉丝数`,`关注人数`,`性别`,`用户等级`,`用户简介`,`用户认证`,`加入日期`,`生日`,`信用等级`,`教育信息`,`用户所在地`) " \
                 "values ({},{},{},{},{},{},{},{},{},{},{},{},{},{});".format(
            index, int(WB_id), "'{}'".format(user_name), "'{}'".format(user_fans_count), "'{}'".format(user_following_count),
