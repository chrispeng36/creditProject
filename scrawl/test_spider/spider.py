...
class Spider:
    def __init__(self, config):
        ...
        '''这里简化成只有txt，只是为了测试，
        我们的代码只是为了得到原创和转发微博，所以txt写入只是为了测试
        '''
    ...
    def get_weibo_info(self) -> list:
        """获取微博信息
        最后返回的weibo_post是一个列表
        如果有元素的话，返回的是weibo_post[i]是每一页对应的键值对
        {"原创微博":original_post},{"转发微博":re_post}
        """
        weibo_post = []
        try:
            since_date = datetime_util.str_to_time(
                self.user_config['since_date'])#爬虫的日期
            now = datetime.now()
            if since_date <= now:
    ...
    def _get_filepath(self, type):
        ...
        try:
            ...
            if FLAGS.output_dir is not None:
                file_dir = FLAGS.output_dir + os.sep + str(self.user.id)
            else:
                file_dir = (os.getcwd() + os.sep + 'weibo' + os.sep + str(self.user.id))
            return file_dir
        except Exception as e:
            logger.exception(e)
    ...
    def get_one_user(self, user_config) -> list:
        """获取一个用户的微博"""
        weibo_list = []
        try:
            # self.get_user_info(user_config['user_uri'])#获取一个用户的基本信息
            # logger.info(self.user)#日志打印用户的基本信息
            # logger.info('*' * 100)

            self.initialize_info(user_config)#初始化用户的配置
            # self.write_user(self.user)#写入用户的基本信息
            # logger.info('*' * 100)
            ...
            for weibos in self.get_weibo_info():
                weibo_list.append(weibos)
                # print(weibos)
                # self.write_weibo(weibos)
                self.got_num += len(weibos)#获取到的微博数目
            if not self.filter:
        ...
        try:
            ...
            for user_config in self.user_config_list:
                if (user_count - user_count1) % random_users == 0:
                    ...
                    random_users = random.randint(*self.random_wait_pages)
                user_count += 1
                weibo_list = self.get_one_user(user_config)
                return weibo_list
        except Exception as e:
            logger.exception(e)
...
def main(_):
    try:
        config = _get_config()#获取config.json的设置的参数
        config_util.validate_config(config)#判断config.json格式是否规范
        wb = Spider(config)
        weibo = wb.start()  # 爬取微博信息

        '''现在我需要的是一个id'''
        # print(config['user_id_list'][0])
        original_post = ''
        repost = ''
        for post in weibo:#post也是个dict，取出就好了
            original_post += post['原创微博']
            repost += post['转发微博']
        '''在这个地方把爬取的数据写入数据库'''
        weibo_idx = int(config['user_id_list'][0])
        sqlStr = "insert into 微博用户发帖新增 (`用户ID`,`原创微博`,`转发微博`)" \
                 "values ({},{},{})".format(
            weibo_idx, "'{}'".format(original_post), "'{}'".format(repost)
        )
        # mysql配置
